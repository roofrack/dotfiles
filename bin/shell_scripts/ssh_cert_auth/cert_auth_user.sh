...
confirm_user() {
  ...
  printf "Selected USER: ${color_bold_underline}%s${normal_color}.\n" "$my_username"
  user_ssh_directory="/home/${my_username}/.ssh"
...
signing_my_own_user_account() {
  ssh-keygen -f "$user_ssh_directory"/"$user_key"
  ssh-keygen -s "$ca_directory"/ca_key_user -I "client-machine" -n "rob,root" -V +22d \
    "$user_key".pub &>/dev/null
  echo "@cert-authority * $(cat ${ca_directory}/ca_key_host.pub)" >"$user_ssh_directory"/known_hosts
}

signing_other_user_accounts() {
  sudo ssh-keygen -f "$user_ssh_directory"/"$user_key"
  echo "@cert-authority * $(cat ${ca_directory}/ca_key_host.pub)" | sudo tee \
    "$user_ssh_directory"/known_hosts &>/dev/null

  sudo cp -a "$user_ssh_directory"/"$user_key".pub .
  # ssh-keygen -s ${ca_directory}/ca_key_user -I "client-machine" -n "$my_username,rob" -V +22d \
  ssh-keygen -s ${ca_directory}/ca_key_user -I "client-machine" -n "rob" -V +22d \
    "$user_key".pub &>/dev/null
  sudo mv "$user_key"-cert.pub "$user_ssh_directory" &>/dev/null

  sudo chown -R "$my_username":"$my_username" "$user_ssh_directory"
  rm -rf "$user_key".pub
}
confirm_user
